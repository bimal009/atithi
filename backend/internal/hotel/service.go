...
func (s *hotelService) Create(ctx context.Context, userID string, req *CreateHotelRequest) (model.Hotel, error) {
	...
	if err != nil {
		s.slog.Error("failed to create hotel", "error", err)
		return model.Hotel{}, err
	}
	...
	s.slog.Info("hotel created", "hotel_id", createdHotel.ID, "owner", userID)
...
func (s *hotelService) FindByID(ctx context.Context, id string) (model.Hotel, error) {
